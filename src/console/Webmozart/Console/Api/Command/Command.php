...
namespace og\console\Webmozart\Console\Api\Command;
...
use og\console\Webmozart\Console\Api\Config\SubCommandConfig;
...
use og\console\Webmozart\Console\Util\ProcessTitle;
use og\http\App;
...
class Command
{
    ...
    private function doHandle(Args $args, IO $io)
    {
        ...
        if(is_callable([$commandHandler, 'init'])) {
            //初始化
            invoke([$commandHandler, 'init'] , [$args, $io, $this]);
        }

        return invoke([$commandHandler, $handlerMethod], [$args, $io, $this]);
    }
}
