...
use og\db\Db;
use og\db\db\exception\BindParamException;
use og\db\db\exception\DataNotFoundException;
use og\db\db\exception\DbException;
use og\db\db\exception\ModelNotFoundException;
use og\db\db\exception\PDOException;
use og\db\Exception;
use og\db\Model;
use og\db\model\Relation;
use og\db\model\relation\OneToOne;
use og\db\Paginator;
...
class Query
{
    ...
    public function paginate($listRows = null, $simple = false, $config = [])
    {
        ...
        /** @var Paginator $class */
        $class = false !== strpos($config['type'], '\\') ? $config['type'] : '\\og\db\\paginator\\driver\\' . ucwords($config['type']);
    ...
    {
        $request = $_REQUEST;
        $config = Db::getConfig('paginate') ?: [];
        if (is_null($pageSize)) {
            $pageSize = array_key_exists('size', $config) ? $config['size'] : 15;
        }
        $var = array_key_exists('pageVar', $config) ? $config['pageVar'] : 'page';
        $pageSize = (int) $pageSize;
        $that = clone $this;
        $total = $that->count();
        $page = isset($request[$var]) ? (int) $request[$var] : 1;
        $maxPage = ceil($total / $pageSize);
        $page = $page > $maxPage ? $maxPage : $page;
        $data = $this->page($page, $pageSize)->select();
        $obj = new \StdClass;
        $obj->currentPage = $page;
        $obj->pageSize = $pageSize;
        $obj->lastPage = $maxPage;
        $obj->total = $total;
        $obj->data = $data;
        return $obj;
    }
